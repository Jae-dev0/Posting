...
test('startup preserves revoked permissions on existing roles', async () => {
  ...
  const names = ['super_admin', 'company_admin', 'marketing_admin', 'employee']
  ...
  stub(prisma.role, 'findUnique', async () => null)
...
function departmentFixture() {
  ...
  return [
    ...
    { ...account, id: 4, roleAssignments: [assignment('employee', 10)] },
    {
      ...account,
      id: 5,
